...
In Adobe Workfront puoi modificare un progetto con la frequenza necessaria. È consigliabile modificare i progetti in modo minimo dopo che lo stato è cambiato in Corrente, per evitare confusione inviando notifiche sulle modifiche all’intero team di progetto. È consigliabile modificare un progetto quando il progetto è nello stato Pianificazione. Per informazioni sul team del progetto, vedi [Panoramica del team del progetto](../../../manage-work/projects/planning-a-project/project-team-overview.md).
...
* Non è possibile modificare progetti che si trovano in un processo di approvazione, ad eccezione del tempo di registrazione.
...
1. Fai clic su **Menu principale** icona ![](assets/main-menu-icon.png) nell’angolo superiore destro di Adobe Workfront, quindi fai clic su **Progetti**.
...
1. Per modificare tutte le informazioni sul progetto, fai clic su **Altro** menu ![](assets/qs-more-menu.png) accanto al nome del progetto, quindi fai clic su **Modifica**.
   ...
   Da un elenco di progetti, seleziona un progetto, quindi fai clic su **Modifica** icona ![](assets/edit-icon.png) nella parte superiore dell’elenco.
...
1. (Condizionale) Se hai selezionato **Altro** menu e quindi **Modifica**, è consigliabile aggiornare le informazioni in una delle seguenti aree elencate nel pannello a sinistra:
...
1. Aggiorna le seguenti informazioni:
   ...
   <table style="table-layout:auto"> 
      ...
      <tr> 
       ...
       <li>Seleziona <strong>Sì</strong> per rendere disponibile nel progetto solo un sottoinsieme dei tipi di ora specifici del progetto, seleziona i tipi di ora che desideri rendere disponibili. (Tenere premuto il tasto Maiusc per selezionare più tipi di ore).</li> 
...
La modifica in blocco di progetti varia a seconda dell’ambiente in cui si sceglie di aggiornarli.

### Modificare i progetti in blocco nell’ambiente di produzione
...
1. Specifica le informazioni su tutti i progetti selezionati nelle sezioni seguenti:
   ...
   * **Panoramica**
      ...
      Per informazioni, vedere [Panoramica](#overview) in questo articolo.

   * **Finanz**

      Per informazioni, vedere [Finanza](#finance) in questo articolo.

   * **Portfolio**

      Per informazioni, consulta la sezione &quot;Associazione progetto&quot; in [Panoramica](#overview) in questo articolo.

   * **Impostazioni**

      Per informazioni, vedere [Impostazioni progetto](#project-settings) in questo articolo.

   * **Accesso**

      Per informazioni, vedere [Accesso](#access) in questo articolo.

   * **Moduli personalizzati**

      Per informazioni, continuare con il passaggio 7 seguente.
   ...
   * **Attività**
      ...
      Per informazioni, vedere [Impostazioni attività](#task-settings) in questo articolo.

   * **Problemi**

      Per informazioni, vedere   [Impostazioni problema](#issue-settings) in questo articolo.

   * **Commento**

      Per informazioni, continuare con il passaggio 9 seguente.
...
1. (Facoltativo) Nell&#39;area Impostazioni (Settings), selezionate una delle seguenti opzioni:

   * **Ricalcolare costi e ricavi**: seleziona questa opzione per ricalcolare costi e ricavi per tutti i progetti selezionati.
   * **Ricalcolare i Timeline**: seleziona questa opzione per ricalcolare le Timeline di tutti i progetti selezionati.
   * **Ricalcolare gli Scorecard**: seleziona questa opzione per ricalcolare i valori della scorecard per tutti i progetti selezionati.
...
1. Clic **Forms personalizzato** per modificare i moduli personalizzati allegati a tutti i progetti selezionati.

   Se i progetti selezionati non hanno moduli personalizzati comuni, in questa sezione non sono elencati moduli.

   È possibile modificare solo i campi dei moduli allegati a tutti i progetti selezionati e per i quali si dispone delle autorizzazioni di modifica.

1. (Facoltativo) Nella sezione Forms personalizzato, seleziona la **Ricalcolare Espressioni Personalizzate** per garantire che tutti i campi personalizzati calcolati presenti nel Forms personalizzato associato ai progetti selezionati siano aggiornati.

   >[!IMPORTANT]
   >
   >È consigliabile non selezionare più di 500 progetti alla volta quando si ricalcolano le espressioni personalizzate.

1. (Facoltativo) Fai clic su **Commento**, quindi seleziona la casella Pubblica un aggiornamento per ciascun progetto e specifica un commento da visualizzare nel flusso di aggiornamenti del progetto nel campo disponibile ed effettua una delle seguenti operazioni:

   * Fai clic su **Persone** icona ![](assets/people-icon-updates-classic.png) per assegnare un tag a un utente che riceverà una notifica relativa al commento.
   * Fai clic su **Blocca** icona ![](assets/lock-icon-open-updates-classic.png) per limitare il commento alle sole persone all&#39;interno dell&#39;azienda.

   Questo commento è visibile a tutti coloro che hanno accesso in visualizzazione al progetto e in visualizzazione Note.

1. Clic **Salva modifiche**.

   Tutte le modifiche apportate sono ora visibili in tutti i progetti selezionati.

<div class="preview">

### Modifica progetti in blocco nell’ambiente di anteprima

Quando modifichi progetti in blocco nell’ambiente di anteprima, tieni presente quanto segue:
...
Per modificare i progetti nell’ambiente di anteprima:
...
1. Fai clic su per selezionare il modulo personalizzato aggiuntivo nella **Aggiungi modulo personalizzato** o **Forms da aggiungere** sottosezioni quando viene visualizzato nell’elenco.

   Quando un modulo personalizzato è già allegato ad alcuni dei progetti selezionati, accanto al nome del modulo è presente un’indicazione del numero di progetti per i quali è già stato selezionato il modulo, al momento dell’aggiunta di un modulo.
