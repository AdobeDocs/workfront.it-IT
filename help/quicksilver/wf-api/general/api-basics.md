...
<pre>GET /attask/api/&lt;versione supportata&gt;/project?id=4c78...54d0&amp;method=get<br>GET /attask/api/&lt;versione supportata&gt;/project/4c78...54d0?method=get</pre>
...
1. Modifica l’URL in /attask/api/&lt;versione supportata>/project/search.\
...
1. Ripristina l&#39;URL **/attask/api/&lt;supported-version>/project/search**.
...
La richiesta /attask/api/&lt;supportata-version>/project?id=... corrisponde alla richiesta `/attask/api/&lt;supported-version&gt;/project/...`.
...
<pre>GET /attask/api/&lt;versione supportata&gt;/task/search?name=Planning<br>&amp;name_Mod=contains<br>&amp;OR:1:portfolio:name=FixedAssets<br>&amp;OR:1:portfolio:name_Mod=eq<br>&amp;OR:1:assignedTo:name=Steve<br>&amp;OR:1:assignedTo:name_Mod=cicontains<br>&amp;OR:2:parent:name=Final Task<br>&amp;OR:2:parent:name_Mod=eq
</pre>
...
<pre>/attask/api/&lt;versione supportata&gt;/user/search?filters={"username":"testuser@workfront.com"}</pre>
...
<pre>/attask/api/&lt;versione supportata&gt;/task/search?map=true</pre>restituisce una risposta indicizzata per ID simile alla seguente:
...
<pre>/attask/api/&lt;versione supportata&gt;/task/search?fields=plannedStartDate,priority</pre>restituisce una risposta simile a quanto segue:
...
<pre>/attask/api/&lt;versione supportata&gt;/issue/search?fields=owner</pre>Se sono necessarie ulteriori informazioni, puoi richiedere un campo nidificato utilizzando la sintassi dei due punti. Ad esempio, la richiesta seguente cerca tutti i problemi insieme al nome, all’ID, al titolo e al numero di telefono del proprietario
<pre>/attask/api/&lt;versione supportata&gt;/issue/search?fields=owner:title,owner:phoneNumber</pre>e restituisce quanto segue: 
...
<pre>/attask/api/&lt;versione supportata&gt;/project/search?fields=tasks</pre>La richiesta seguente ottiene le assegnazioni delle attività:
<pre>/attask/api/&lt;versione supportata&gt;/task/search?fields=assignments</pre>
...
<pre>/attask/api/&lt;versione supportata&gt;/project/search?fields=DE:CustomText</pre>che restituirebbe
...
<pre>/attask/api/&lt;versione supportata&gt;/project/search?fields=parameterValues</pre>restituisce dati simili ai seguenti:
...
<pre>/attask/api/&lt;versione supportata&gt;/work/myWork</pre>Le query denominate supportano la richiesta del parametro fields per recuperare campi aggiuntivi. Alcune query denominate accettano anche filtri aggiuntivi. Per un elenco delle query denominate consentite per un oggetto, consulta la scheda Azione per l’oggetto nella [API Explorer](https://developer.adobe.com/workfront/api-explorer/).
...
<pre>GET /attask/api/&lt;versione supportata&gt;/project/count?status=CUR</pre>restituisce il numero di risultati nel formato seguente:
...
<pre>GET /attask/api/&lt;versione supportata&gt;/ora/report?project:name_1_GroupBy=true&amp;hours_AggFunc=sum</pre>che restituisce il seguente risultato
...
<pre>GET /attask/api/&lt;versione supportata&gt;/project/123abcxxxxxxxxxxxxxxxxxxxxxxxx?method=put &amp;updates={ accessRules: [ {accessorID: 'abc123', accessorObjCode: 'USER', coreAction: 'VIEW'} ] }</pre>In alternativa, per condividere solo con una nuova persona e mantenere intatte le autorizzazioni esistenti:
<pre>GET /attask/api/&lt;versione supportata&gt;/project/123abcxxxxxxxxxxxxxxxxxxxxxxxx/share?method=put&amp;accessorID=abc123&amp;accessorObjCode=USER&amp;coreAction=VIEW</pre>Per recuperare le regole di accesso esistenti:
<pre>GET /attask/api/&lt;versione supportata&gt;/project/123abcxxxxxxxxxxxxxxxxxxxxxxxxxx?fields=accessRules:*</pre>
...
<pre>POST /attask/api/&lt;versione supportata&gt;/project?name=Nuovo progetto</pre>La risposta include il progetto appena creato, il nuovo ID e tutti gli altri campi specificati.
...
<pre>POST /attask/api/&lt;versione supportata&gt;/upload</pre>L’API prevede che il tipo di contenuto sia multipart/form-data. Il nome del parametro per il file deve essere uploadedFile. Il server restituisce i seguenti dati JSON:
...
<pre>POST /attask/api/&lt;versione supportata&gt;/document?updates={<br> name: aFileName,<br> handle: abc...123, (handle dal caricamento del file)<br> docObjCode: PROJ, (o TASK, OPTASK, ecc.)<br> objID: abc...123,<br> currentVersion:{version:v1.0,fileName:aFileName}<br>}</pre>
...
<pre>PUT /attask/api/&lt;versione supportata&gt;/project/4c7...?name=Nome nuovo progetto <br>PUT /attask/api/&lt;versione supportata&gt;/project?id=4c7...&amp;name=Nome nuovo progetto</pre>Poiché l’aggiornamento richiede un ID, questa operazione non riuscirà (senza inserimento) se l’oggetto non esiste sul server.
...
<pre>PUT /attask/api/&lt;versione supportata&gt;/project/4c7...?updates= <br>{<br> name: "Nuovo nome progetto", <br> status: "CUR", <br> ... <br>}</pre>
...
<pre>PUT /attask/api/&lt;versione supportata&gt;/task/4c7...?updates= <br>{<br> assegnazioni: [ <br> { <br> assignedToID: "2222...54d0, <br> assignmentPercent: 50.0 <br> },{ <br> roleID: "1111...54d0"<br> } <br> ] <br>}</pre>
...
<pre>PUT /attask/api/&lt;versione supportata&gt;/project/4c7...?updates= <br>{ <br> queueDef: { <br> isPublic: 1 <br> } <br>}</pre>
...
<pre>PUT /attask/api/&lt;versione supportata&gt;/project/4c7...?action=calculattimeline<br><br>or<br><br>PUT /attask/api/&lt;versione supportata&gt;/project/4c7.../calculattimeline </pre>
...
<pre>PUT /attask/api/&lt;versione supportata&gt;/task/4c7.../move?projectID=5d8...</pre>Un esempio per ogni tipo di azione è fornito qui: (??)
<pre>PUT /attask/api/&lt;versione supportata&gt;/project/1234/approveApproval<br><br>PUT /attask/api/&lt;versione supportata&gt;/project/1234/calculateFinance<br><br>PUT /attask/api/&lt;versione supportata&gt;/project/1234/calculattimeline<br><br>PUT /attask/api/&lt;versione supportata&gt;/project/1234/calculatdataExtension<br><br>PUT /attask/api/&lt;versione supportata&gt;/project/1234/recallApproval<br><br>PUT /attask/api/&lt;versione supportata&gt;/project/1234/rejectedApproval<br><br>PUT /attask/api/&lt;versione supportata&gt;/task/1234/move<br><br>PUT /attask/api/&lt;versione supportata&gt;/workitem/1234/markViewed</pre>Solo l’azione Sposta richiede l’identificazione di attributi aggiuntivi per specificare il progetto in cui deve essere spostato l’elemento di lavoro.

Di seguito è riportato un esempio di ciascun tipo di azione: 
<pre>PUT /attask/api/&lt;versione supportata&gt;/project/1234?method=put&amp;updates={accessRules:[{accessorID: 'abc123', accessorObjCode: 'USER', coreAction: 'VIEW'}]}</pre>

### Condividere di oggetti

L’esempio seguente illustra la sintassi per la condivisione di un progetto con un team:
<pre>PUT /attask/api/&lt;versione supportata&gt;/project/123abcxxxxxxxxxxxxxxxxxxxxxxxx/share?accessorID=123abcxxxxxxxxxxxxxxxxxxxxxxxx&amp;accessorObjCode=TEAMOB</pre>Durante la modifica di un oggetto, puoi sostituire tutte le regole di accesso su un oggetto eseguendo una richiesta PUT e inviando aggiornamenti simili a quelli riportati di seguito:
<pre>PUT /attask/api/&lt;versione supportata&gt;/project/123abcxxxxxxxxxxxxxxxxxxxxxx?method=PUT&amp;updates={accessRules:[{accessorID:'123abcxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx',accessorObjCode:'TEAMOB',coreAction:'VIEW'}]}</pre>Nell’esempio seguente viene illustrata la sintassi per lo spostamento di un’attività da un progetto a un altro:
<pre>PUT /attask/api/&lt;versione supportata&gt;/task/4c7.../move?projectID=5d8...</pre>
...
<pre>PUT /attask/api/&lt;versione supportata&gt;/proj?updates=[{"name":"Test_Project_1"},{"name":"Test_Project_2"}]&amp;method=POST&amp;apiKey=123ab-cxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx</pre>oppure <pre>PUSH /attask/api/&lt;versione supportata&gt;/proj?updates=[{"name":"Test_Project_1"},{"name":"Test_Project_2"}]&amp;method=POST&amp;apiKey=123ab-cxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx</pre>che produce un risultato simile al seguente:
...
<pre>PUT /attask/api/&lt;versione supportata&gt;/proj?Umethod=PUT&amp;updates=[{"ID":"123abcxxxxxxxxxxxxxxxxxxxxxxxxxxxx","name":"Test_Project_1_ Edit"},{"ID":"123abcxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx","name":"Test_Project_2_Edit"}]&amp;apiKey=123abcxxxxxxxxxxxxxxxxxxxxxxxxxxxx</pre>che produce un risultato simile al seguente:
