---
content-type: api
navigation-topic: Event Subscriptions resources
description: Risorse sottoscrizioni eventi
author: Becky
feature: Workfront API
role: Developer
source-git-commit: 3e339e2bfb26e101f0305c05f620a21541394993
workflow-type: tm+mt
source-wordcount: '46'
ht-degree: 0%

---

# Abbonamenti eventi

Di seguito sono riportate diverse risorse su Abbonamenti eventi in Adobe Workfront:

* [API sottoscrizione evento](../../wf-api/general/event-subs-api.md)
* [Best practice per l&#39;abbonamento agli eventi](../../wf-api/general/event-sub-best-practice.md)
* [Requisiti di consegna per abbonamento eventi](../../wf-api/general/setup-event-sub-endpoint.md)
* [Campi risorse sottoscrizione evento](../../wf-api/api/event-sub-resource-fields.md)
* [Nuovi tentativi di sottoscrizione evento](../../wf-api/api/event-sub-retries.md)
* [Filtrare i messaggi di abbonamento agli eventi](../../wf-api/api/filter-event-sub-messages.md)
* [Formato del messaggio in uscita per le sottoscrizioni di eventi](../../wf-api/api/message-format-event-subs.md)
* [Domande frequenti - Abbonamenti agli eventi](../../wf-api/general/event-subs-faq.md)

