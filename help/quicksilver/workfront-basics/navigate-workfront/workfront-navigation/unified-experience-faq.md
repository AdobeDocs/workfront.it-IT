...
title: Domande frequenti su Adobe Unified Experience
...
| La Guida è accessibile tramite il [!UICONTROL menu principale] e l&#39;area di navigazione superiore | La Guida è accessibile tramite [!UICONTROL Menu principale] e la barra di navigazione [!DNL Workfront] |
| Il visualizzatore delle prove si apre in una nuova scheda | Il visualizzatore delle prove si apre in Workfront |
| URL utilizzato per accedere a Workfront: `experience.adobe.com` | URL utilizzato per accedere a Workfront: `(CompanyName).my.workfront.adobe.com` |
| Il formato della data (ad esempio MM/GG/AAAA) si basa sulle impostazioni della lingua di Unified Experience. Se l&#39;utente non ha aggiornato le impostazioni della lingua, verranno utilizzate `en-US` impostazioni. | Il formato della data (ad esempio MM/GG/AAAA) si basa sulle preferenze del browser |

{style="table-layout:auto"}

## Domande frequenti

### Come posso determinare se sto utilizzando Adobe Unified Experience o Adobe Workfront?

Per determinare se la tua organizzazione utilizza Adobe Unified Experience, esamina l&#39;URL utilizzato per accedere a Workfront.

| URL | Adobe Experience |
|------------|------------|
| (CompanyName).my.workfront.com | Esperienza Workfront |
| experience.adobe.com | Esperienza unificata Adobe |

### Come è possibile ottenere ulteriori informazioni su [!DNL Adobe Admin Console]?

Per informazioni su [!DNL Admin Console], vedere gli articoli seguenti:
...
### In che modo ciò influisce su quelli configurati con SSO?

SSO è configurato in [!DNL Admin Console] ed ereditato dall&#39;applicazione [!DNL Workfront].

### SSO con il nostro [!DNL Active Directory] interno sarà ancora un&#39;opzione con il sistema Identity Management (IMS) di Adobe?

IMS sostituisce SSO e funziona per lo più allo stesso modo. Tutte le autorizzazioni utente sono concesse e fornite in [!DNL Adobe Admin Console] e l&#39;utente visualizzerà la schermata di accesso di [!DNL Adobe] in cui potrà scegliere &quot;[!UICONTROL Account personale]&quot; o &quot;[!UICONTROL Account aziendale]&quot; per accedere (se disponi di [!DNL Active Directory], la maggior parte eseguirà l&#39;accesso con un account aziendale).

### Per gli utenti che non utilizzano SSO, l&#39;URL di accesso [!DNL Workfront] cambia?

L’URL di accesso cambierà; tuttavia, il vecchio URL reindirizzerà al nuovo URL di accesso, quindi dovresti istruire nuovamente gli utenti su dove andare.
...
### Ci sarà un momento in cui i reindirizzamenti saranno disabilitati? Oppure sarà sempre possibile digitare in my.company.workfront.com?
...
[!DNL Admin Console] offre agli amministratori un notevole controllo sugli elementi a cui gli utenti possono o non possono accedere. Ogni volta che un utente esterno richiede l&#39;accesso, deve creare un ID [!DNL Adobe], che invia un&#39;e-mail all&#39;amministratore. L&#39;amministratore può quindi accettare o rifiutare l&#39;accesso a un prodotto e definire a cosa può o non può accedere per i prodotti di proprietà di quell&#39;organizzazione. L&#39;amministratore di sistema di [!DNL Workfront] può quindi accedere all&#39;area [!UICONTROL Utenti] di [!DNL Workfront] per creare autorizzazioni più granulari per l&#39;utente esterno.

### Gli amministratori dei gruppi vengono utilizzati per creare persone in [!DNL Workfront]. Con lo spostamento in [!DNL Experience Cloud], gli amministratori dei gruppi potranno ancora creare persone?

Sì, la creazione dell&#39;utente è ancora possibile tramite [!DNL Workfront]. Tali utenti possono essere aggiunti automaticamente a [!DNL Experience Cloud]. Puoi anche configurare gli amministratori dei gruppi come proprietari del prodotto in [!DNL Admin Console] per consentire loro di assegnare [!DNL Workfront] agli utenti.

### Qual è la scadenza per passare a [!DNL Experience Cloud]?

Nessuna scadenza da spostare in [!DNL Adobe Experience Cloud]. Stiamo lavorando con i clienti per farli scegliere quando sono pronti a fare la mossa.

### Il team di supporto dovrà intervenire per apportare le modifiche?

Se il team di supporto è responsabile della creazione di nuovi utenti, dovrà acquisire familiarità con le interfacce [!DNL Admin Console] utilizzate per la creazione di utenti e assegnare un&#39;autorizzazione a Workfront. In caso contrario, è probabile che non si verifichino cambiamenti significativi per il team di supporto interno.

### In che modo questo influisce sulle integrazioni disponibili tramite la funzione API?

Il percorso URL esistente continuerà a essere disponibile per il traffico API. Non dovresti dover fare nulla per aggiornare gli endpoint nelle integrazioni. Tuttavia, l’accesso diretto tramite nome utente e password non sarà supportato: è necessario utilizzare una chiave API, ad eccezione dei connettori [!DNL Workfront Fusion].
...
[!DNL Workfront] utenti di dispositivi mobili non devono essere interessati dalla migrazione a [!DNL Adobe Unified Experience].

### JumpSeat non funziona con [!DNL Adobe Unified Experience]. Come posso risolvere il problema?

JumpSeat funziona con [!DNL Adobe Unified Experience], ma richiede un aggiornamento della configurazione. Utilizzando il pannello di amministrazione JumpSeat, modificare l&#39;URL dell&#39;applicazione da `workfront.com` a fine con `.workfront.adobe.com`
