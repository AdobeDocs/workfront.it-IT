...
<tr> 
   <td> <p>Capacità</p> </td> 
   <td> <p>Il tempo disponibile di una risorsa quando può essere allocata al lavoro. Vedere "Disponibilità". </p></td> 
  </tr>
...
<tr> 
   ...
   <td><p>Qualsiasi oggetto Workfront o le informazioni ad esso associate, come visualizzato nel database. </p>
   <p>Ad esempio, "project", "user", "hour" sono sia oggetti Workfront che campi. "Nome", "stato", "proprietario", "data di inizio" sono campi Workfront associati agli oggetti di cui sopra. </p>

<p>Quando si fa riferimento agli oggetti, è possibile utilizzare in modo intercambiabile i termini "oggetti" e "campi".</p>
...
<p>Nel reporting di testo, i campi fanno riferimento agli oggetti o alle relative informazioni come vengono visualizzati nel database.</p>
   <p>A volte il nome visualizzato nell'interfaccia utente è diverso dal nome del campo nel database. Ad esempio, "problema" è il nome dell'oggetto nell'interfaccia di Workfront, ma "opTask" è il nome dell'oggetto (o il campo) nel database Workfront. </p> 
   <p> È importante utilizzare il campo così come viene visualizzato nel database quando si scrive un rapporto in modalità testo, una visualizzazione, un filtro o un raggruppamento o si crea un campo calcolato.</p>

<p>Per ulteriori informazioni, consulta <a href="../../../wf-api/general/api-explorer.md">Esplora API</a> e <a href="../../../reports-and-dashboards/reports/text-mode/understand-text-mode.md">Panoramica della modalità testo</a>.</p>

<p>Per impostazione predefinita, Workfront viene fornito con un set di campi che definiscono sia gli oggetti che le relative informazioni. È inoltre possibile creare campi personalizzati per definire oggetti, ma non oggetti personalizzati.</p> 
   </td> 
  </tr>
...
<p>L’ [!UICONTROL FTE] deve essere un numero decimale massimo di 1 e non può essere 0. </p>
   <p> Un FTE di [!UICONTROL] di 1 (che è l’impostazione predefinita per un campo FTE di un utente, come definito nel suo profilo) significa che una risorsa (utente o ruolo) funziona per l’intero numero di ore, in base alla pianificazione che ne calcola la disponibilità. </p>
   <p>L’amministratore di Workfront decide quale pianificazione utilizzare per determinare la disponibilità dell’utente.  </p>
   <ul>
   <li> Quando si utilizza la [!UICONTROL Default Schedule] (Pianificazione predefinita), Workfront utilizza l’[!UICONTROL FTE] dell’utente che si trova nel suo profilo per calcolare la disponibilità. </li>
   <li> Quando si utilizza la pianificazione dell’utente, Workfront utilizza il valore di time off dell’utente, il valore di [!UICONTROL Work Time] e le ore di [!UICONTROL Default Schedule] dell’utente per calcolare l’FTE dell’utente. </li> </ul>
...
<p><b>NOTA</b></p>
   <p>Per tutti i calcoli nel Planner scenario, Workfront utilizza il seguente valore: 1 [!UICONTROL FTE] = 8 ore.</p>
   <p>Per ulteriori informazioni, consulta <a href="../../../scenario-planner/get-started-with-scenario-planning.md">Guida introduttiva a [!UICONTROL Scenario Planner]</a>. </p>
   </td> </tr> 
   </tbody> 
   </table>
...
<table style="table-layout:auto"> 
 ...
 <tbody> 
  ...
  <tr> 
   ...
   <td> <p>Le informazioni visualizzate in [!DNL Adobe Workfront] è rappresentato da oggetti memorizzati nel [!DNL Workfront] database. Gli oggetti sono ciò che guida le informazioni in Workfront. Alcuni esempi di oggetti sono:</p> 
    <ul> 
     <li>[!UICONTROL Portfoli]</li> 
     <li>[!UICONTROL Programmi]</li> 
     <li>[!UICONTROL Projects]</li> 
     ...
     <li>[!UICONTROL Tassi di fatturazione]</li> 
     <li>Modelli [!UICONTROL]</li> 
     <li>[!UICONTROL Attività modello]</li>

<p><b>NOTA</b></p>
  <p>Questa non è una lista completa. </p>
...
<table style="table-layout:auto"> 
 ...
 <tbody> 
  ...
  <tr> 
   ...
   <td> <p>Intervallo di tempo entro il quale è pianificato che si verifichi un evento. Quando crei progetti, attività o problemi in [!DNL Workfront], stabilisci le date di inizio e di fine pianificate e l’intervallo di tempo pianificato durante il quale si verificano. Questi valori rappresentano l'intenzione originale o la stima del tempo necessario al completamento di un elemento. </p></td> 
  </tr> 
  <tr> 
...
<p>Rappresenta la percentuale del tempo equivalente a tempo pieno ([!UICONTROL FTE]) che l'utente è disponibile per il lavoro effettivo, senza includere il sovraccarico. Il [!UICONTROL Work Time] deve essere un numero decimale massimo di 1 e non può essere 0. Ad esempio, una disponibilità del 20% per il lavoro effettivo sarebbe 0,2.</p>
   </p>L’impostazione predefinita del campo è 1 e indica che un utente spende l’intero [!UICONTROL FTE] per un lavoro effettivo correlato al progetto.  </p>
   <p>Il sistema utilizza questo numero per calcolare la disponibilità dell'utente per il lavoro effettivo relativo al progetto. </p>
   <p> Le eccezioni di pianificazione e il tempo di inattività potrebbero influire anche sulla capacità dell'utente. </p>
   <p>Per ulteriori informazioni sulla creazione di pianificazioni in Workfront, vedi <a href="../../../administration-and-setup/set-up-workfront/configure-timesheets-schedules/create-schedules.md">Creare una pianificazione</a>. </p>
    <p>Workfront calcola la disponibilità di un utente in base alle preferenze di Gestione risorse nell’area Configurazione di . Per ulteriori informazioni, consulta <a href="../../../administration-and-setup/set-up-workfront/configure-system-defaults/configure-resource-mgmt-preferences.md/">Configurare le preferenze di Gestione risorse</a>. </p> 
   <p>È possibile aggiornare l’[!UICONTROL Work Time] di un utente quando lo si modifica o crea. Per informazioni, consulta <a href="../../../administration-and-setup/add-users/create-and-manage-users/edit-a-users-profile.md">Modificare il profilo di un utente</a></p> 
   <b>SUGGERIMENTO</b> 
   <p>Imposta il valore [!UICONTROL Work Time] su 1 per indicare che l’utente è disponibile per il lavoro relativo al progetto per l’intero equivalente a tempo pieno.</p> 
   </td> 
  </tr> 
  <tr> 
   <td>[!UICONTROL Orario di lavoro]</td> 
