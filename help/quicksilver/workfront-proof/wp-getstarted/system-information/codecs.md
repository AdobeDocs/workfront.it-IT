---
content-type: reference
product-previous: workfront-proof
product-area: documents;system-administration
navigation-topic: system-information
title: Codec in [!DNL Workfront Proof]
description: Codec in [!DNL Workfront Proof]
author: Courtney
feature: Workfront Proof, Digital Content and Documents
exl-id: f760dbaf-edbb-49ec-a329-54727372bdc6
TQID: https://experienceleague.adobe.com/oaiiey4FOEV8tgyFgTsNcqndQTj0RF3OAXxTGWTS--M
product_v2:
  - id: c4a86a5d-6562-4fc6-aa00-bfa25833aed9
feature_v2:
  - id: d968a1bc-9a90-4926-a531-bcf272c32aad
role_v2:
  - id: b69b2659-1057-424e-8fc5-ed9e016dc554
topic_v2:
  - id: eddd9b14-83bd-4ff4-9072-54a4a484abb7
source-git-commit: 55a9d9feae8cc1128e3427a8874414ba734dd467
workflow-type: tm+mt
source-wordcount: 101
ht-degree: 0%

---

# Codec in [!DNL Workfront Proof]

>[!IMPORTANT]
>
>Questo articolo fa riferimento alle funzionalità nel prodotto autonomo [!DNL Workfront Proof]. Per informazioni sulla verifica all&#39;interno di [!DNL Adobe Workfront], vedere [Verifica](../../../review-and-approve-work/proofing/proofing.md).

## Codec supportati

* [!DNL 3ivx]
* [!DNL 4X Movie]
* [!DNL Alaris VideoGramPiX]
* [!DNL Alparysoft] codec senza perdita di dati
* [!DNL American Laser Games MM Video]
* [!UICONTROL Video AMV]
* [!DNL Apple QuickDraw]
* [!DNL ASUS] V1
* [!DNL ASUS] V2
* [!DNL ATI VCR-2]
* [!DNL ATI VCR1]
* [!DNL Auravision AURA]
* [!DNL Auravision Aura 2]
* Video di [!DNL Autodesk Animator Flic]
* [!DNL Autodesk RLE]
* [!DNL Avid Meridien Uncompressed]
* [!DNL AVImszh]
* [!DNL AVIzlib]
* Video AVS ([!DNL Audio Video Standard])
* [!DNL Beam Software VB]
* Video di [!DNL Bethesda VID]
* Video di [!UICONTROL Bink]
* [!DNL Blackmagic 10-bit]
* [!DNL Broadway MPEG Capture Codec]
* Codec [!DNL Brooktree 411]
* [!DNL Brute Force & Ignorance]
* [!DNL CamStudio]
* [!DNL Camtasia Screen Codec]
* [!DNL Canopus HQ Codec]
* [!DNL Canopus Lossless Codec]
* Video di [!DNL CD Graphics]
* Video [!DNL Chinese AVS] ([!DNL AVS1-P2], profilo [!DNL JiZhun])
* [!DNL Cinepak]
* [!DNL Cirrus Logic AccuPak]
* [!DNL Creative Labs Video Blaster Webcam]
* [!DNL Creative YUV] (CYUV)
* Video di [!DNL Delphine Software International CIN]
* [!UICONTROL Animazione pittura deluxe]
* [!DNL DivX ;-)] ([!DNL MPEG-4])
* [!DNL DNxHD] (VC3)
* [!DNL DV] (video digitale)
* [!DNL Feeble Files]/[!DNL ScummVM DXA]
* #1 codec video [!DNL FFmpeg]
* [!DNL Flash Screen Video]
* [!DNL Flash Video] (FLV) / [!DNL Sorenson Spark] / [!DNL Sorenson H.263]
* [!DNL Forward Uncompressed Video Codec]
* [!DNL fox motion video]
* [!DNL FRAPS: Realtime Video Capture]
* [!DNL GeoVision Advanced MPEG-4]
* Codec [!DNL GoToMeeting] (v1 e v2)
* [!DNL H.261]
* [!DNL H.263], [!DNL H.263+]
* [!DNL H.264] / [!DNL AVC] / [!DNL MPEG-4 AVC] / [!DNL MPEG-4 part 10]
* [!DNL HuffYUV]
* [!DNL I263]
* [!DNL IBM Ultimotion]
* [!DNL IBM UltiMotion]
* Video di [!DNL id Quake II CIN]
* Video di [!DNL id RoQ]
* [!DNL IFF ByteRun1]
* [!DNL IFF ILBM]
* Codec [!DNL Independent JPEG Group's]
* [!DNL Infinite Video PSI_V]
* [!DNL Intel Indeo 2]
* [!DNL Intel Indeo 3]
* [!DNL Intel Indeo 4.1]
* [!DNL Intel Indeo 5]
* [!DNL Interplay C93]
* Video di [!DNL Interplay MVE]
* Codec video di [!DNL Karl Morton]
* [!DNL Kega Game Video]
* [!DNL kensington webcam]
* [!DNL Lagarith Lossless Video Codec]
* [!DNL LCL (LossLess Codec Library) MSZH]
* [!DNL Lead CMW wavelet 2.0]
* [!DNL LOCO]
* [!DNL LZO compressed]
* [!DNL Microsoft RLE]
* [!DNL Microsoft Video 1]
* [!DNL Midivid 3]
* [!DNL Mimic]
* [!DNL Miro VideoXL]
* [!DNL MJPEG (Motion JPEG)]
* Video di [!DNL Motion Pixels]
* [!DNL Motion Wavelets]
* [!DNL Moyea Flash to Video Converter]
* Video di [!DNL MPEG-1]
* Video di [!DNL MPEG-2]
* [!DNL MPEG-4 part 2]
* [!DNL MPEG-4 part 2] ([!DNL Microsoft] varianti 1-3)
* [!DNL MPEG-PES]
* [!DNL MSS2]
* [!DNL MSU Screen Capture Lossless Codec]
* Video di [!DNL Nintendo Gamecube THP]
* [!DNL nokia eti camcorder eyecon]
* [!DNL NuppelVideo/RTJPEG]
* [!DNL On2 VP3]
* [!DNL On2 VP4]
* [!DNL On2 VP5]
* [!DNL On2 VP6]
* [!DNL On2 VP6A] (con [!DNL alpha channel])
* [!DNL On2 VP7]
* Video di [!DNL Power VideoWorks]
* [!DNL ProRes 422]
* Video di [!DNL Psygnosis YOP]
* [!DNL Q-team QPEG]
* [!DNL QPEG]
* Video di [!DNL QuickTime 8BPS]
* Video di [!DNL QuickTime Animation (RLE)]
* [!DNL QuickTime Graphics (SMC)]
* [!DNL QuickTime] video ([!DNL RPZA])
* [!DNL RealVideo 1.0]
* [!DNL RealVideo 2.0]
* [!DNL RealVideo 3.0]
* [!DNL RealVideo 4.0]
* Video di [!DNL RL2]
* [!DNL Schrödinger] ([!DNL Schroedinger]), [!DNL Dirac]
* Video di [!DNL Sierra VMD]
* [!DNL sif1 alpha4]
* Video di [!DNL Smacker]
* [!DNL SMPTE VC-1]
* [!DNL SMV2]
* [!DNL Snow]
* Codec [!DNL SoftMedia ViVD V2]
* [!DNL Sony Digital Video (DV)]
* [!DNL Sony PlayStation MDEC (Motion DECoder)]
* [!DNL Sorenson Vector Quantizer 1] / [!DNL Sorenson Video 1] / [!DNL SVQ1]
* [!DNL Sorenson Vector Quantizer 3] / [!DNL Sorenson Video 3] / [!DNL SVQ3]
* [!DNL SP4x]
* [!DNL SP5x]
* [!DNL Streambox ACT-L2]
* [!DNL SVQ3]
* [!DNL Telegeny VDTZ]
* [!DNL Theora]
* Video di [!DNL Tiertex Limited SEQ]
* [!DNL TrueMotion 1.0]
* [!DNL TrueMotion 2.0]
* [!DNL UCOD-ClearVideo]
* Codec avanzato [!DNL VDOWave 3]
* Codec [!DNL VDOWave]
* [!DNL Verint Video Manager]
* [!DNL Vianet Lsvx Video]
* [!DNL VMware Screen Codec] / [!DNL VMware Video]
* [!DNL VP8]
* Video di [!DNL VQA (Vector Quantized Animation)]
* [!DNL VSS Codec Light]
* [!DNL VSS Wavelet Video Codec]
* [!DNL WebTrain Communication] registratore di schermo senza perdita
* Codec [!DNL wincam screen capture]
* [!DNL Windows Media Screen Codec 2]
* [!DNL Windows Media Video (WMV)]
* [!DNL Windows Media Video (VC-1) Advanced Profile]
* [!DNL Windows Media Video 7 (WMV 7, WMV1)]
* [!DNL Windows Media Video 8 (WMV 8, WMV2)]
* [!DNL Windows Media Video 9 (WMV 9, WMV3)]
* [!DNL Windows Media Video Adv]
* [!DNL Windows Screen Video]
* Codec [!DNL Winnov Videum winx]
* Codec [!DNL Winnov Videum wnv1]
* [!DNL Winnov WNV1]
* [!DNL WorldConnect Wavelet Video]
* [!DNL XAN Video]
* Video di [!DNL xfire]
* [!DNL xiricam Veo PC Camera]
* [!DNL Xvid (MPEG-4)]
* [!DNL YUV]
* [!DNL zdsoft screen recorder]
* [!DNL Zip Motion Blocks Video]

## Codec non supportati

* [!DNL Apple Intermediate]
* [!DNL ProRes 4444] ([!DNL ProRes 422] supportati)
* [!DNL HDV 720p60]
* [!DNL Go2Meeting3 (G2M3)]
* [!DNL Go2Meeting4 (G2M4)]
* [!DNL ER AAC LD] (resiliente agli errori, variante a basso ritardo di [!DNL AAC])
* [!DNL REDCODE]
