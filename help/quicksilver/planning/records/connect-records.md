...
<td> 
   ...
   <ul><li><p>Una licenza Adobe Experience Manager Assets e un’integrazione tra AEM Assets e Workfront per collegare le risorse AEM ai tipi di Planning.</p>
...
1. Iniziare a digitare il nome di un record nella casella di ricerca, quindi selezionarlo quando viene visualizzato nell&#39;elenco
   ...
   * I campi collegati vengono compilati con le informazioni dei record collegati, se sono stati aggiunti campi di ricerca collegati al momento della connessione dei tipi di record.
   ...
   >* Non è possibile aggiungere campi typeahead di Workfront come campi di ricerca. Nell’ambiente di produzione, sono inclusi campi di tipo persone come Proprietario progetto o Sponsor progetto. <span class="preview">È possibile aggiungere campi di tipo persone come campi di ricerca nell&#39;ambiente di anteprima.</span>
   >
...
1. (Condizionale) Dopo aver connesso il tipo di record selezionato con Experience Manager, passa al campo connesso di un record e fai clic sul campo oppure fai clic su **Connetti** per aggiungere risorse Experience Manager al record, quindi fai clic sull&#39;icona **+**.
   ...
   Viene visualizzata la casella Contenuto verificato.

   ![Casella Contenuto verificato per connessioni record AEM](assets/content-advisor-assets-nothing-selected.png)
...
1. Fai clic su per selezionare alcuni dei seguenti tipi di risorse dall&#39;area **Assets**:
...
1. Fare clic su **Frammenti di contenuto** per selezionare frammenti di contenuto da aggiungere al campo record collegato.
...
1. (Facoltativo) Vai al tipo di record da cui hai collegato Experience Manager e fai clic sul nome di una risorsa nel campo del record collegato. I dettagli Experience Manager della risorsa vengono visualizzati in una finestra a comparsa.
...
1. (Facoltativo) Fai clic sull&#39;icona **Apri in una nuova scheda** ![Apri i dettagli in una nuova scheda](assets/open-details-in-a-new-tab-icon.png) per aprire la pagina del record in una nuova scheda del browser.
1. (Facoltativo e condizionale) Fare clic sul nome di un tipo di record nella breadcrumb dell&#39;intestazione della pagina di record per accedere a un altro tipo di record nella stessa gerarchia. Per poter visualizzare le gerarchie nel breadcrumb, è necessario che esistano per il tipo di record del record che si sta connettendo. Per informazioni, vedere [Creare gerarchie area di lavoro](/help/quicksilver/planning/architecture/create-workspace-hierarchies.md).
