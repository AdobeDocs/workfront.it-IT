...
Prima di iniziare a utilizzare la modalità testo in un report o un elenco, assicuratevi sempre di avere familiarità con la sintassi della modalità testo di Workfront.
...
La modifica di un filtro in modalità testo è identica per report ed elenchi. L’accesso al filtro da un rapporto o da un elenco è diverso.
...
1. Esegui una delle operazioni seguenti:
   ...
   Se utilizzi il generatore di filtri legacy o un report, fai clic su **Aggiungi regola filtro** per iniziare ad aggiungere le condizioni del filtro. Quindi, fai clic su **Passa alla modalità Testo** e poi su **Modifica modalità Testo** sul lato destro del generatore.

1. Aggiungere istruzioni di filtro utilizzando la modalità testo. Ogni istruzione filtro può contenere le seguenti righe e informazioni aggiuntive:
   ...
   <table style="table-layout:auto"> 
    ...
    <tbody> 
     <tr> 
      <td><b>Linea/informazioni filtro</b></td> 
      <td><b>Esempio</b></td> 
     </tr> 
     <tr> 
   ...
   Quando si filtrano gli stati, è necessario utilizzare il codice a tre lettere dello stato e non il nome.</p> </td>
   </tr> 
     <tr> 
      <td> <p>Modificatore nome campo e uguale a. Indica le condizioni che il campo in base al quale si sta filtrando deve soddisfare.</p> <p>Questa riga è obbligatoria.</p> </td> 
      <td> <p><code>&lt;field name in camel case&gt;_Mod=&lt;modifier value&gt;</code> </p> <p>Per indicare che lo stato delle attività per le quali si applica il filtro deve essere uguale a In corso, utilizzare la riga seguente oltre a quella precedente:</p> <p><code>status_Mod=in</code> </p> <p>Se il modificatore è un intervallo, sono disponibili due linee per indicare il modificatore.</p> 
       <div> <span class="autonumber"><span><b>ESEMPIO </b></span></span> 
        <p>Questo filtro in modalità testo consente di cercare le attività in corso, con una data di completamento pianificata entro il mese corrente e assegnate a un utente con un GUID specifico:</p> 
     ...
     <tr> 
      <td> <p>Operatore di rendiconto. Per impostazione predefinita, ogni istruzione di filtro è connessa dall'operatore "AND". Questa opzione non viene visualizzata nell’interfaccia della modalità testo. È inoltre possibile aggiungere un operatore "OR" tra due istruzioni per indicare che si desidera filtrare gli oggetti che soddisfano una o l'altra delle due condizioni.</p> <p>Gli operatori di filtro sono necessari solo per i filtri con più di un'istruzione.</p> <p>Suggerimento   
        <ul> 
      ...
      <td> <p><code>&lt;first field name in camel case&gt;=&lt;value&gt;</code> </p> <p><code>&lt;first field name in camel case&gt;_Mod=&lt;modifier value&gt;</code> </p> <p><code>OR:1:&lt;second field name in camel case&gt;=&lt;value&gt;</code> </p> <p><code>OR:1:&lt;second field name in camel case&gt;_Mod=&lt;modifier value&gt;</code> </p> 
       <div> <span class="autonumber"><span><b>ESEMPIO </b></span></span> 
        <p>Per filtrare le attività con stato In corso o Data di completamento pianificata odierna, utilizzare quanto segue: </p> 
        <p><code>status=INP</code> </p> 
        <p><code>status_Mod=in</code> </p> 
...
1. Per aggiungere un’istruzione di filtro connessa dall’operatore &quot;OR&quot;, effettua le seguenti operazioni:
   ...
   1. Aggiungere una seconda riga e digitare OR:1: seguito dall&#39;oggetto, dal modificatore e dal codice del modificatore. Per definire il modificatore per la riga di codice che fa riferimento a tutti gli stati dei task ad eccezione di Nuovo, utilizzare la linea di modificatore seguente:
      ...
      >[!NOTE]
      >
      >È possibile avere più istruzioni &quot;OR&quot; nello stesso filtro. Ogni volta che viene visualizzata una nuova istruzione &quot;OR&quot;, il numero dopo &quot;OR:&quot; aumenta.
      >
      >Per filtrare le attività con stato In corso o assegnate all&#39;utente che ha effettuato l&#39;accesso o per le quali è stata impostata la Data di completamento pianificata oggi, utilizzare quanto segue:
      >
      >`status=INP`
      >`status_Mod=in`
...
1. Fai clic su **Applica** o **Fine** per salvare le modifiche alla modalità testo e continuare a modificare il report o il filtro.
