...
## Creare o modificare un filtro nel generatore standard {#create-filter-in-standard-builder}

È possibile creare filtri in elenchi e rapporti nei modi seguenti:
...
## Creare o modificare un filtro nel generatore di versioni beta

Quando crei filtri utilizzando diverse interfacce, considera quanto segue:

* Puoi trovare il generatore di versioni beta nelle stesse posizioni in cui trovi l’interfaccia filtro standard per le aree elencate nella tabella precedente.
* Puoi passare dall’interfaccia standard a quella beta builder e viceversa, dove è disponibile l’opzione beta.
* Dopo aver abilitato il generatore beta in un’area, questa è l’esperienza predefinita per tutte le aree in cui è disponibile. Ad esempio, se abiliti il Generatore di versioni beta in un elenco di progetti, è l’esperienza predefinita per la creazione di attività e la creazione di filtri negli elenchi.
* Puoi creare filtri utilizzando l’interfaccia del generatore di versioni beta nei seguenti modi:
...
* I filtri salvati sono disponibili in entrambi i generatori, indipendentemente dall’esperienza utilizzata per generarli originariamente. Ad esempio, se hai creato un filtro utilizzando il generatore standard, puoi trovarlo e modificarlo anche nell’interfaccia del generatore beta.
   ...
   >[!TIP]
   >
   >Un filtro &quot;All&quot; non è incluso nel generatore beta perché tutte le voci dell’elenco vengono visualizzate quando non vengono applicati filtri. Fai clic su **Cancella tutto** in alto a destra nel generatore per cancellare eventuali filtri attivi e visualizzare tutti gli elementi. Se **Cancella tutto** è oscurato, non vengono applicati filtri.

* I generatori standard e beta hanno una sintassi leggermente diversa quando si creano filtri con più istruzioni che combinano gli operatori AND e OR. Di conseguenza, questi filtri possono essere visualizzati in modo diverso quando si passa da un generatore all’altro.

   >[!INFO]
   >
   >Esiste lo scenario seguente:
   >
   >1. Utilizza il generatore di versioni beta per creare un filtro con la seguente sintassi:
   >
   >   `(A OR B) AND C`
   >
   >1. Torna al generatore standard e modifica il filtro utilizzando la sintassi del generatore standard come descritto in [Creare o modificare un filtro nel generatore standard](#create-filter-in-standard-builder) in questo articolo. La sintassi per il generatore standard visualizza le istruzioni del filtro come segue:
   >
   >   `A AND C`
   >   `OR`
   >   `B AND C`
   ...
   >1. Torna al generatore beta. L’istruzione filter viene visualizzata in base alla logica supportata nel generatore standard, come descritto in precedenza.
   ...
   >   `B AND C`
   > 
   >   Questo accade perché il filtro è stato modificato nell&#39;interfaccia standard.

Crea un filtro utilizzando l’interfaccia del generatore di versioni beta:
...
1. Fai clic sul pulsante **Filtro** icona ![Icona Filtro](assets/filter-nwepng.png), quindi abilita **Impostazione beta** ![Impostazione beta](assets/beta-toggle-white-on-existing-filters.png) per accedere al generatore beta. È disabilitata per impostazione predefinita.
   ...
   Quindi, accetta il contratto beta se necessario. Devi solo concordare una volta e il generatore beta rimane abilitato.

   Viene aperta l’interfaccia del generatore di filtri beta.

   >[!TIP]
   >
   >L’intestazione dell’interfaccia del generatore di filtri diventa blu quando si abilita il generatore di filtri beta. Dopo aver abilitato l’interfaccia del generatore beta, Workfront la mantiene abilitata per tutte le aree in cui è disponibile.
...
1. Fai clic sul menu a discesa del campo per visualizzare un elenco dei campi utilizzati di recente e dei campi consigliati per filtrare. I campi suggeriti sono attualmente visualizzati nell’elenco a cui si sta filtrando.
   ...
   Puoi anche selezionare **Campi di ricerca** per visualizzare un elenco di tutti i campi che è possibile filtrare in base a. I campi nella ricerca avanzata sono raggruppati per categoria di oggetti.
...
1. Fai clic sul menu a discesa del modificatore per selezionare un modificatore. Il modificatore predefinito è &quot;Uguale a&quot;.
   ...
   >[!TIP]
   >
   >Durante la creazione del filtro, i risultati vengono visualizzati immediatamente nell’elenco. Se il pannello dei filtri copre l’elenco, puoi chiuderlo per visualizzarne la visualizzazione. Le informazioni immesse rimangono nel generatore di versione beta quando riapri il pannello.
...
1. (Facoltativo) Fai clic su **Modalità testo** per continuare a creare il filtro utilizzando la modalità testo.
   ...
   >[!TIP]
   >
   >È consigliabile creare il maggior numero possibile di filtri utilizzando l’interfaccia del generatore di versioni beta e utilizzando la modalità testo solo quando è necessario apportare modifiche al filtro supportate solo in modalità testo.
...
1. Fai clic su **Esci da modalità testo** per tornare all’interfaccia del generatore beta.

   >[!WARNING]
   >
   >Alcune istruzioni della modalità di testo non sono supportate nel generatore beta o nell’interfaccia standard. Se si esce dalla modalità di testo dopo la creazione di questi tipi di istruzioni, potrebbe essere generato un messaggio di avviso.
